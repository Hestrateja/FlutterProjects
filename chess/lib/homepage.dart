...
class _HomePageState extends State<HomePage>{
  ...
  var pieces = [
    ...
    //open spaces -> x = blank, o = possible, k = possible kill
    [16,'x', '', '', ''],
    [17,'x', '', '', ''],
    [18,'x', '', '', ''],
    [19,'x', '', '', ''],
  ...
  void tappedPawn(int index){
    ...
    setState(() {
      ...
      if(pieces[index][2] == 'white'){
        //chevk if the square infront of the pawn is blank, if it's not...
        ...
        //if there is an enemt piece diagonal to the currently tapped pawn,
      ...
      if(pieces[index][2] == 'black'){
        //chek if the square infront of the pawn is blank, if it's not...
        if(pieces[index+row][1] != 'x'){
          //do nothing
        }
         // only when pawns are in the 2nd row can they move 2 squares
  ...
  void tappedRook(int index){
    ...
    // add all the possitions to possibleMoves until there is a piece in the way
  ...
  void tappedKnight(int index){
    //total possible moves for knight, but we can't include everything in this list
    //depending on where we are, otherwise the move will spill over to the next column
    List<int> possibleMoves = [
      ...
      for(int i = 0; i < possibleMoves.length; i++){
        ...
        if(withinTheBoard(possibleMoves[i])){
          ...
          colorOfCurrentlySelectedPiece != pieces[possibleMoves[i]][2]){
            //then it is a possible kill move
 ...
 void tappedKing(int index){
  ...
  //king can go up, down, right, and 4 diagonals
